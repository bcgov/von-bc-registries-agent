...
try:
    ...
    (post_credential_pipeline, success) = data_integration.pipelines.find_node(['initialization_and_load_tasks','bc_reg_credential_poster_lear']) 
    if success:
        creds_ct = 0
        with EventProcessor() as eventprocessor:
            creds_ct = eventprocessor.get_outstanding_creds_record_count(system_type_cd=lear_system_type)
        while 0 < creds_ct:
