...
with EventProcessor() as event_processor:
    ...
    sql2 = """
    SELECT record_id, system_type_cd, corp_num, corp_state, corp_json->>'corp_typ_cd' as corp_typ_cd, last_event->>'event_id' as last_event_id, last_event->>'event_date' as last_event_date, entry_date, process_date 
    FROM corp_history_log
    WHERE process_date is not null and (process_msg != 'Withdrawn' or process_msg is null)
      AND record_id > (select COALESCE(MAX(LAST_CORP_HISTORY_ID), 0) from CORP_AUDIT_LOG)
    ORDER BY record_id;
    """
