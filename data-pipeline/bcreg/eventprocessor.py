...
class EventProcessor:
    ...
    def generate_address_credential(self, corp_num, corp_info, office, address, dba_corp_num, dba_name):
        ...
        addr_cred['end_date'] = None

        return addr_cred

    # generate credentials for the provided corp
    def generate_credentials(self, cur, system_typ_cd, prev_event_id, last_event_id, corp_num, corp_info):
        ...
        for office in corp_info['office']:
            if 'office_typ_cd' in office:
                if 'delivery_addr' in office and 'local_addr' in office['delivery_addr']:
                    ...
                    self.insert_json_credential(cur, system_typ_cd, prev_event_id, last_event_id, corp_num, 
                                            addr_credential, addr_schema, addr_version, addr_cred)
                # TODO not sure if we want both delivery and mailing addresses
                #if 'mailing_addr' in office and 'local_addr' in office['mailing_addr']:
                #    addr_cred = self.generate_address_credential(corp_num, corp_info, office, office['mailing_addr'], "", "")
                #    self.insert_json_credential(cur, system_typ_cd, prev_event_id, last_event_id, corp_num, 
                #                            addr_credential, addr_schema, addr_version, addr_cred)
        ...
        if 'parties' in corp_info:
            for party in corp_info['parties']:
                for dba_name in party['dba_names']:
                    dba_cred = {}
            ...
            with BCRegistries() as bc_registries:
                for i,corp in enumerate(corps): 
                    ...
                    if generate_creds:
                        ...
                        cur = self.conn.cursor()
                        self.generate_credentials(cur, corp['SYSTEM_TYPE_CD'], corp['PREV_EVENT_ID'], corp['LAST_EVENT_ID'], corp['CORP_NUM'],
                                                corp_info)
                        cur.close()
                        cur = None
