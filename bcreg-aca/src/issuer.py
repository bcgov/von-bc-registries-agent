            ...
            for credential_type in issuer_info["credential_types"]:
                ...
                if register_schema:
                    ...
                    if labels or descriptions:
                        ctype_config["labels"] = {}
                        for key in labels:
                            if key not in ctype_config["labels"]:
                                ctype_config["labels"][key] = {}
                            ctype_config["labels"][key]["label"] = labels[key]
                        for key in descriptions:
                            if key not in ctype_config["labels"]:
                                ctype_config["labels"][key] = {}
                            ctype_config["labels"][key]["description"] = descriptions[key]
    ...
    def run(self):
        ...
        try:
            ...
            if result_available and not result_available.wait(MAX_CRED_RESPONSE_TIMEOUT):
                ...
                add_credential_timeout_report(credential_exchange_id, cred_data["thread_id"])
                LOGGER.error(
